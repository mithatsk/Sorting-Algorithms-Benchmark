...
int main(){
    ...
    maxval=500;
    minval=-50;
	double * insertionAverage =(double *) malloc(10*sizeof(double));
    double * insertionMin =(double *) malloc(10*sizeof(double));
    double * insertionMax =(double *) malloc(10*sizeof(double));
    ...
    for(a=0,len=16000;a<10;len=len*1.2,a++){
		...
		if(x==9){
			//	insertionMin[a] = FindMin(arrayDiff,10); //sorun find fonksiyonlarýnda
    			insertionAverage[a] = (FindAverage(arrayDiff,10));
    		//	insertionMax[a] = FindMax(arrayDiff,10);
    		//	printf("insertionMin[%d] = %f\n",a,insertionMin[a]);
	  			printf("%f\n",insertionAverage[a]);
	  		//	printf("insertionMax[%d] = %f\n",a,insertionMax[a]);	  			
			}	
		}
