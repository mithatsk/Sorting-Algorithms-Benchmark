...
int main(){
    ...
    maxval=500;
    minval=-50;
	double * combAverage =(double *) malloc(10*sizeof(double));
    double * combMin =(double *) malloc(10*sizeof(double));
    double * combMax =(double *) malloc(10*sizeof(double));
    ...
    for(a=0,len=16000;a<10;len=len*1.2,a++){
		...
		if(x==9){
			//	combMin[a] = FindMin(arrayDiff,10); //sorun find fonksiyonlarýnda

			
    			combAverage[a] = FindAverage(arrayDiff,10);
    
    		//	combMax[a] = FindMax(arrayDiff,10);
    		//	printf("combMin[%d] = %f\n",a,combMin[a]);
	  			printf("%f\n",combAverage[a]);
	  		//	printf("combMax[%d] = %f\n",a,combMax[a]);	  			
			}	
