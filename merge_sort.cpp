...
int main(){
    ...
    maxval=500;
    minval=-50;
	double * mergeAverage =(double *) malloc(10*sizeof(double));
    double * mergeMin =(double *) malloc(10*sizeof(double));
    double * mergeMax =(double *) malloc(10*sizeof(double));
    ...
    for(a=0,len=16000;a<10;len=len*1.2,a++){
		...
		if(x==9){
			//	mergeMin[a] = FindMin(arrayDiff,10); //sorun find fonksiyonlarýnda
			mergeAverage[a] = FindAverage(arrayDiff,10);
			/*if(a==0){
			
    			mergeAverage[a] = FindAverage(arrayDiff,10);
    			}
    			else{
    				mergeAverage[a] = ((FindAverage(arrayDiff,10))/(pow(1.2,a)*log(pow(1.2,a))));
				}*/
    		//	mergeMax[a] = FindMax(arrayDiff,10);
    		//	printf("mergeMin[%d] = %f\n",a,mergeMin[a]);
	  			printf("%f\n",mergeAverage[a]);
	  		//	printf("mergeMax[%d] = %f\n",a,mergeMax[a]);	  			
			}	
